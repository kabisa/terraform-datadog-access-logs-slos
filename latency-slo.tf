locals {
  ...
  latency_slo_numerator   = coalesce(var.latency_slo_custom_numerator, "sum:${var.slo_metric_prefix}requests.lt.${var.latency_slo_latency_bucket}ms.count{${local.latency_slo_filter}}.as_count()")
  latency_slo_denominator = coalesce(var.latency_slo_custom_denominator, "sum:${var.slo_metric_prefix}requests.count{${local.latency_slo_filter}}.as_count()")
}


resource "datadog_service_level_objective" "latency_slo" {
  count       = var.latency_slo_enabled ? 1 : 0
  name        = "${local.service_display_name} - ${var.log_source_name} - Latency SLO"
  type        = "metric"
...
module "latency_slo_burn_rate" {
  ...
  name  = "${var.log_source_name} - Latency SLO - Burn Rate"
